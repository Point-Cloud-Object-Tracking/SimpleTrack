...
from mot_3d.config import Config
...
class KittiLoader:
    def __init__(
        self,
        configs: Dict[str, Any], # A yaml file. See SimpleTrack/configs/kitti_configs/*.yaml
        ...
        start_frame: int | None, # What frame to start at in the sequence of frame. If None, then start at frame 0
    ):
        ...
        assert 0 <= self.cur_frame < self.max_frame, f"Start frame {self.cur_frame} is out of range"
    ...
    def __next__(self):
        ...
        det_types = [inst_types[i] for i in range(len(bboxes)) if inst_types[i] in self.type_token]
        ...
        if 'velos' in self.dets.keys():
            ...
            tmp = [np.array(cur_frame_velos[i]) for i in range(len(bboxes)) if inst_types[i] in self.type_token]
        ...
        if self.nms:
            dets, det_types, velos = self.frame_nms(dets, det_types, velos, self.nms_thres)
        ...
        result: DatasetSample = {
            ...
            'aux_info': {
                'velos': velos,
                'is_key_frame': self.is_key_frames[idx]
            },
            'pc': pc
        }
    ...
    def frame_nms(self,
                ...
                thres: float) -> Tuple[List[BBox], List[int], Optional[List[float]]]:
