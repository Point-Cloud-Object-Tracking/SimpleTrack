...
class WaymoLoader:
    def __init__(
        ...
        self.ts_info = json.load(
            open(
                os.path.join(data_folder, "ts_info", "{:}.json".format(segment_name)),
                "r",
            )
        )
    ...
    def __next__(self):
        ...
        result["dets"] = [
            BBox.bbox2world(result["ego"], BBox.array2bbox(b)) for b in selected_dets
        ]
        ...
        if "velos" in self.dets.keys():
            ...
            result["aux_info"]["velos"] = [
                np.array(cur_frame_velos[i])
                for i in range(len(bboxes))
                if inst_types[i] in self.type_token
            ]
            result["aux_info"]["velos"] = [
                utils.velo2world(result["ego"], v) for v in result["aux_info"]["velos"]
            ]
        else:
