...
parser.add_argument(
    "--start_frame", type=int, default=0, help="start at a middle frame for debug"
)
parser.add_argument(
    "--obj_types", default="car,bus,trailer,truck,pedestrian,bicycle,motorcycle"
)
# paths
parser.add_argument(
    "--config_path",
...
def load_gt_bboxes(data_folder, type_token, segment_name):
    gt_info = np.load(
        os.path.join(data_folder, "gt_info", "{:}.npz".format(segment_name)),
        allow_pickle=True,
    )
    ...
    gt_ids, gt_bboxes = utils.inst_filter(
        ids, mot_bboxes, inst_types, type_field=type_token, id_trans=True
    )
...
def sequence_mot(
    ...
    for frame_index in range(data_loader.cur_frame, frame_num):
        if frame_index % 10 == 0:
            print(
                "TYPE {:} SEQ {:} Frame {:} / {:}".format(
                    obj_type, sequence_id, frame_index + 1, frame_num
                )
            )
    ...
    for obj_type in obj_types:
        ...
        for file_index, file_name in enumerate(file_names[:]):
            if file_index % process != token:
                continue
            print(
                "START TYPE {:} SEQ {:} / {:}".format(
                    obj_type, file_index + 1, len(file_names)
                )
            )
            ...
            ids, bboxes, states, types = sequence_mot(
                configs,
                data_loader,
                obj_type,
                file_index,
            ...
            for frame_index in range(frame_num):
                ...
                for i in range(id_num):
                    ids[frame_index][i] = "{:}_{:}".format(
                        file_index, ids[frame_index][i]
                    )
